...
contract SandwichGold is AssetMarket, IPlayer {
    ...
    function createBundle(
        ...
        // Randomly create 4 different types of salmonella bundles
        if (builderIdx % 4 == 0) {
            // Revert, so that an empty bundle is returned to the builder
            require(false, "I have no bundle");
        } else if (builderIdx % 4 == 1) {
            // Create a bundle with an amount greater than player reserves
            ...
            // Create a bundle with an invalid asset index
            bundle = PlayerBundle({swaps: new SwapSell[](1)});
            bundle.swaps[0] = SwapSell({
                fromAssetIdx: GOLD_IDX,
                toAssetIdx: 100,
            ...
            // Create a bundle with a lot of meaningless swaps, to bait builders into miscalculating profits
